...
%% 计算时差
%% 信号频差补偿
y2_xz=y_dn2.*exp(-1j*2*pi*fd*t_d);
...
T_pie=Nf/(M_t*fs_d);
Nf_pie=int32(fs_pie*T_pie);
win_t = hamming(R12_M_L);
R12_w = R12_M.*win_t';
R12_pie=[R12_w zeros(1,Nf_pie-R12_M_L)];
r12_pie=ifft(R12_pie,Nf_pie);
[r_max,n_max]=max(abs(r12_pie(1:length(r12_pie)/2+1)));
td=(n_max-1)/fs_pie;